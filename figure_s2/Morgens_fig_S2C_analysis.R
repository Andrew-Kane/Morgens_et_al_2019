...
process <- function(data,r){
  ...
  ctrl <- ddply(.data = subset(data, drug == 'Retro2' & concentration == 0), .variables ='replicate', summarize,
...
fitter <- function(vals,d){
  fitted <- drm(formula = mean_normalized ~ concentration, data = subset(vals,drug == d), fct = LL.4(names = c("Slope", "Lower", "Upper", "EC50")))
  fitted
}

drm(formula =   mean_normalized ~ concentration , data = Retro2, fct = LL.4(names = c("Slope", "Lower", "Upper", "EC50")))
summary(Retro2.fit)
...
files = list.files(pattern="*s2c_rep*")
...
temporary <- processed[which(processed$drug == 'Retro2' & processed$concentration == 0),]
temporary$drug <- 'DHQZ36.1'
processed <- rbind(processed, temporary)

#Creates a list of all strains in the processed dataset.
drugs <- unique(processed$drug)

#Creates a list of drc fits for each strain in the processed dataset.
fits <- lapply(drugs,fitter,vals=processed)

#Plots the Retro2 fit.
index <- which(drugs=='Retro2')
plot(fits[[index]], type = c("bar"),ylim=c(0.45,1))
plot(fits[[index]], add=TRUE, type = c("average"))

#Plots the DHQZ36.1 fit.
index <- which(drugs=='DHQZ36.1')
plot(fits[[index]], add=TRUE, type = c("bar"), col="red")
plot(fits[[index]], add=TRUE, type = c("average"),col="red",pch=22)

#ggsave('~/path/to/Fig/S2C/plot.pdf', device = cairo_pdf(width = 10, height = 5))
