...
process <- function(data,r){
  ...
  statdata <- ddply(.data=normdata, .variables = c('concentration','strain','replicate'), summarize,
                    mean_normalized = mean(normalized))
  plotdata <- ddply(.data=statdata, .variables = c('concentration','strain'),summarize,
...
setwd("/Users/akane/R_testing/Morgens_et_al_2019_2019/figure_s4-2/")

#Creates a list of csv files to be processed in this script.
files = list.files(pattern="*s4-2f_rep*")
...
ggplot(plotdata, aes(x = factor(strain,level=c('K562WT','K562A149V','K562A149A')), y = mean, fill = factor(concentration))) +
